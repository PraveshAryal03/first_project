...
class _ProfileInfoPageState extends State<ProfileInfoPage> {
    ...
    try {
      ...
      if (doc.exists) {
        ...
        setState(() {
          ...
          _recommendedCalories = data['recommendedCalories'] ?? 0;
          _recommendedProtein = data['recommendedProtein'] ?? 0;
          _recommendedCarbs = data['recommendedCarbs'] ?? 0;
          _recommendedFats = data['recommendedFats'] ?? 0;
        });

        // If we already have saved targets, push them into the provider
        if (_recommendedCalories > 0) {
          final tracker = Provider.of<CalorieTrackerProvider>(
            context,
            listen: false,
          );
          ...
          // Recalculate from fields (will also update provider)
  ...
  void _calculateRecommendations() {
    ...
    switch (_goal) {
      case 'Lose Weight':
        ...
        break;
      case 'Gain Weight':
        targetCalories = tdee + 500; // surplus
        break;
      case 'Maintain':
      default:
        targetCalories = tdee;
    ...
    // 🔥 Immediately update provider so HomePage reflects this
    final tracker = Provider.of<CalorieTrackerProvider>(context, listen: false);
  ...
  Future<void> _save() async {
    ...
    // Recalculate recommendations from current inputs
    _calculateRecommendations();
    ...
    // 1) UPDATE PROVIDER IMMEDIATELY so HomePage sees new targets
    try {
      final tracker = Provider.of<CalorieTrackerProvider>(
        context,
        listen: false,
      ...
      debugPrint('Error updating CalorieTrackerProvider: $e');
    }
    ...
    // 3) FIRE-AND-FORGET FIREBASE (doesn't block UI)
    () async {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Profile saved locally, sync failed: $e')),
          );
        }
      }
    }();

    // 4) NOW UPDATE UI & POP THE SCREEN
    if (!mounted) return;
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        'These targets are calculated based on the information you provided. You can adjust them later if needed.',
